...
6. **One command, "Export to LinkML".** Finds the `skiss` blocks in the active note, compiles them with `toLinkML` and `serialize`, and writes `<note>.linkml.yaml` next to the note via the vault API. Diagnostics go to a notice.
