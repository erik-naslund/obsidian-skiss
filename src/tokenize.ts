...
// SPEC §4: the name productions, and the whitespace that separates them.
const CLASS_NAME = /^[A-Z][A-Za-z0-9]*$/;
const FIELD_NAME = /^[a-z][A-Za-z0-9]*$/;
const SYSTEM_NAME = /^[A-Za-z][A-Za-z0-9_-]*$/;
const ENUM_VALUE = /^[A-Za-z0-9][A-Za-z0-9_-]*$/;
const WORD = /[\p{L}\p{N}_-]+/uy;
const TRAILING_WS = /[ \t]+$/;

/** SPEC §3.2: the primitives and the aliases that name them. */
...
function scan(head: string): Raw[] {
  ...
  while (i < head.length) {
    ...
    if (word !== null) {
      raws.push({ text: word[0], from: i, to: i + word[0].length, word: true });
      i += word[0].length;
      continue;
    }
    if (c === '[' && head[i + 1] === ']') {
...
function nameTypeTokens(word: Raw): Token[] {
  if (PRIMITIVES.includes(word.text)) {
